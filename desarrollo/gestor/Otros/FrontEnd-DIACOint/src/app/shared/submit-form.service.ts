...
export class SubmitFormService {
	...
	aconf; aconf_paramgen; aconf_usuarios; aconf_sistema; aconf_colas; aconf_aprobacionpp; aconf_catalogos; aconf_presencial;
	atcon_verificador;
  ...
  SetInitial(formsubmit){
		...
		if(this.srv_atencion_consumidor !=0){
			...
			return true;
		}else if(this.aconf != 0 || this.aconf_catalogos != 0 || this.aconf_presencial != 0){
			this.router.navigate(['/mainAtCon']);
			return true;
		}else{
			this.Limpiar();
