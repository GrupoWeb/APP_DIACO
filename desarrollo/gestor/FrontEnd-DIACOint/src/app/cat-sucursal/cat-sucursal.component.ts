...
this._proveedorService.findByNitProveedor(this.selectedProveedor.nit_proveedor).subscribe((retvalue)=>{
  if(retvalue["reason"] == 'OK'){
    ...
    if(tempstr != null)	{
      ...
      this.selectedSucursal.id_proveedor=this.proveedorArray[0].id_proveedor;
      this.selectedSucursal.nombre_proveedor= this.proveedorArray[0].nombre;
      this.GetList(this.selectedSucursal.id_proveedor);
